      ...
      <ModalContent maxW='500px'>
		...
		return (
            	  <div key={factura._id} className='factura'>
            	    <span>{numero}</span> <span>{fecha}</span><span>{totalImporte} €</span><a href={`${socketURL}/api/user/bills/${_id}`} target="_blank" rel="noreferrer"><img className='descarga-icon' src={descargaIcon} alt='descargar' loading="lazy" /></a>
                  </div>
		)
	     })}
            </>
